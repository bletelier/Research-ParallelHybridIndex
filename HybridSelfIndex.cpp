...
void HybridSelfIndex::locate(uchar *pat, uint m, ulong *nOcc, ulong **occ, int mode, int mode2, double &tseq, double &tpo, double &tso, double &tps, vector<ulong> &fo, vector<ulong> &so){
	if (m==1)
		...
		if (m<=M)
		{
			if(mode >= 1)
			{
				nt = mode;
				if(mode2 == 1)
					newParallelLocateUptoM_1(pat, m, nOcc, occ, tseq, tpo, tso, tps, fo, so);				
				else if(mode2 == 2)
					newParallelLocateUptoM_2(pat, m, nOcc, occ, tseq, tpo, tso, tps, fo, so);				
				else if(mode2 == 3)
					newParallelLocateUptoM_3(pat, m, nOcc, occ, tseq, tpo, tso, tps, fo, so);				
				else if (mode2 == 0)
					parallelLocateUptoM(pat,m,nOcc,occ,tseq, tpo, tso, tps, fo, so);
			}
			else
			{
				locateUptoM(pat, m, nOcc, occ, tseq, tpo, tso, tps);
			}
		}
		else{
			if (m<=(M<<1))
...
void HybridSelfIndex::locateUptoM(uchar *pat, uint m, ulong *nOcc, ulong **occ, double &tseq, double &tpo, double &tso, double &tps){
	string query = string((char *)pat);
	double t;
	t = omp_get_wtime();
	auto list = sdsl::locate(FMI, query.begin(), query.begin()+m);
...
void HybridSelfIndex::locateSecOccAuxiliar(ulong l, ulong r, ulong posX, uint m, ulong *nOcc, vector<ulong> &occ, ulong &cont_tid,ulong *currN){
	ulong pos, x, y, phra, xPr;
...
void HybridSelfIndex::parallelLocateUptoM(uchar *pat, uint m, ulong *nOcc, ulong **occ, double &tseq, double &tpo, double &tso, double &tps, vector<ulong> &fo, vector<ulong> &so){
	string query = string((char *)pat);
	double t;
	t = omp_get_wtime();
	auto list = sdsl::locate(FMI, query.begin(), query.begin()+m);
	...
	if (nLoc){
		ulong *A;
		vector<ulong> cont_tid(nt,0);
		vector<ulong> a(nt,0);
		vector<ulong> ps_tid(nt+1,0);
		...
		#pragma omp parallel shared(cont_tid, ps_tid,nLoc, list, nt,cs, occ, A)
		{
			...
			for(long i=0; i<nn; i++){
				if (findPredecesor(a_tid[tid][i], &r)){
					locateSecOccAuxiliar(0, r, a_tid[tid][i], m, nOcc, a_tid[tid], a[tid],&currN);
				}
			}
			#pragma omp barrier
			#pragma omp single
			{	
				...
				for(int i = 0; i < nt; ++i)
				{
					ps_tid[i+1] = ps_tid[i] + a[i] +  cont_tid[i];
				}
		...
		*occ = A;
		...
		/*cout << "T loop + rec = " << t << endl;
		cout << "------------" << endl;*/
		calculate_occs_per_thread(cont_tid, fo);
		calculate_occs_per_thread(a, so);	
	}
	else
		*nOcc=0;
...
#pragma region newLocate - DP

void HybridSelfIndex::newLocateSecOccAuxiliar(ulong l, ulong r, ulong posX, uint m, vector<vector<ulong>> &occ, ulong *cont){
	ulong pos, x, y, phra, xPr;
	int tid = omp_get_thread_num();
	...
	xPr = getPosPhraT(phra)+posX-x;
	occ[tid].push_back(xPr);
	(cont[tid])++;
	if (findPredecesor(xPr, &x))
	{
		#pragma omp task shared(occ) if(x > 512) affinity(cont[tid])
			newLocateSecOccAuxiliar(0, x, xPr, m, occ, cont);	
	}
	/*bool left;
	left = (pos-1-l) >= (r-pos+1);*/
	
	if (pos>l){
		//pragma omp task shared(occ) if(left and (pos-1-l) > 16)
		newLocateSecOccAuxiliar(l, pos-1, posX, m, occ, cont);
	}
	if (pos<r)
	{
		//pragma omp task shared(occ) if(!left and (r-pos+1) > 16)
		newLocateSecOccAuxiliar(pos+1, r, posX, m, occ, cont);
	}
	//#pragma omp taskwait
}

void HybridSelfIndex::newParallelLocateUptoM_1(uchar *pat, uint m, ulong *nOcc, ulong **occ, double &tseq, double &tpo, double &tso, double &tps, vector<ulong> &fo, vector<ulong> &so){
	double t;
	t = omp_get_wtime();
	string query = string((char *)pat);
	auto list = sdsl::locate(FMI, query.begin(), query.begin()+m);
	int nLoc = list.size();
	if (!nLoc) {*nOcc = 0; return;}
	ulong *A;
	ulong *cont_tid = new ulong[nt];
	vector<long> ps_tid(nt+1,0);
	vector<vector<ulong>> a_tid(nt);
	long long cs = nLoc/(nt);
	ulong *a = new ulong[nt];
	vector<vector<ulong>> ax_tid(nt);
	*nOcc = 0;
	t = omp_get_wtime() - t;
	tseq += t;
	t = omp_get_wtime();
	#pragma omp parallel shared(cont_tid, ps_tid,nLoc, list, nt,cs, occ, A, nOcc) proc_bind(close)
	{
		int tid = omp_get_thread_num();
		cont_tid[tid] = 0;
		a[tid] = 0;
		...
		for(long i=b; i< e; i++){
			if (isPrimary(list[i], m, &pr, &dx)){
				pxx = getPosPhraT(pr) - dx;
				a_tid[tid].push_back(pxx);
				cont_tid[tid]++;
				if (findPredecesor(pxx, &r)){
					newLocateSecOccAuxiliar(0, r, pxx, m, a_tid, a);
				}
			}else{
				if (dx){
					if (BL_il[pr-1]){
...
void HybridSelfIndex::newParallelLocateUptoM_2(uchar *pat, uint m, ulong *nOcc, ulong **occ, double &tseq, double &tpo, double &tso, double &tps, vector<ulong> &fo, vector<ulong> &so){
	double t;
	t = omp_get_wtime();
	...
	*nOcc = 0;
	...
	t = omp_get_wtime();
	#pragma omp parallel shared(cont_tid, ps_tid,nLoc, list, nt,cs, occ, A, nOcc)
	{
		int tid = omp_get_thread_num();
		cont_tid[tid] = 0;
		a[tid] = 0;
...
void HybridSelfIndex::newParallelLocateUptoM_3(uchar *pat, uint m, ulong *nOcc, ulong **occ, double &tseq, double &tpo, double &tso, double &tps, vector<ulong> &fo, vector<ulong> &so){
	double t;
	t = omp_get_wtime();
	string query = string((char *)pat);
	auto list = sdsl::locate(FMI, query.begin(), query.begin()+m);
	...
	*nOcc = 0;
	...
	#pragma omp parallel shared(cont_tid, ps_tid,nLoc, list, nt,cs, occ, A, nOcc)
	{
		...
		{	
			...
			t = omp_get_wtime();
			ulong r;
			for(long t_id=0;t_id<nt; t_id++){
				for(long i=0; i<cont_tid[t_id]; i++){
					if (findPredecesor(a_tid[t_id][i], &r)){
						newLocateSecOccAuxiliar(0, r, a_tid[t_id][i], m, a_tid, a);
					}
				}
			}
			t = omp_get_wtime() - t;
...
void HybridSelfIndex::calculate_occs_per_thread(vector<ulong> &cont, vector<ulong> &res){
	for (int i = 0; i < nt; ++i)
		res[i] = cont[i];
}

void HybridSelfIndex::calculate_occs_per_thread(ulong *cont, vector<ulong> &res){
	for (int i = 0; i < nt; ++i)
		res[i] = cont[i];
}
